...
INSERT INTO reviews(series_id, reviewer_id, rating) VALUES
    ...
    select title, round(avg(rating),2) as rating from series 
    JOIN reviews on series.id=reviews.series_id group by title order 
    by rating asc;
...
case when ifnull(AVG(rating),0)>0 then 'ACTIVE'
else 'INACTIVE'
end as genre
from reviewers 
